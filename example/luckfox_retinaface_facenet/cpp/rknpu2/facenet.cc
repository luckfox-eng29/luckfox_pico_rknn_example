...
void letterbox(cv::Mat image, unsigned char * input_data)
{
    ...
    //resize 
    cv::Mat letterboxImage(160, 160, image.type(), cv::Scalar(0, 0, 0));
    ...
    //cv::imwrite("letterbox.jpg",letterboxImage);
    ...
    for (int y = 0; y < letterboxImage.rows; ++y) 
    {
      for (int x = 0; x < letterboxImage.cols; ++x)
       {
            cv::Vec3b pixel =letterboxImage.at<cv::Vec3b>(y, x);
            input_data[(y * 160 + x) * 3 + 0] = pixel[2]; // Red
            input_data[(y * 160 + x) * 3 + 1] = pixel[1]; // Green
            input_data[(y * 160 + x) * 3 + 2] = pixel[0]; // Blue
        }
    }
}
